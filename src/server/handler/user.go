...
func RegisterUser(ctx echo.Context) error {
	...
	if len(user.AlipayAccount) == 0 {
		return ctx.JSON(errors.NewTaskServerError(http.StatusForbidden, fmt.Errorf("must provide alipayAccount")))
	}
	user.Registered = true
	oldUser, err := service.GetUserByName(user.Name)
	if err != nil {
		return ctx.JSON(errors.NewTaskServerError(http.StatusNotFound, fmt.Errorf("Error in get user : %s, '%s' ", user.Name, err)))
	}
	if oldUser.Registered {
		return ctx.JSON(errors.NewTaskServerError(http.StatusConflict, fmt.Errorf("Error in register user : %s, '%s' ", user.Name, "user has been registered")))
	}
	newUser, err := service.UpdateUser(user)
	if err != nil {
		return ctx.JSON(errors.NewTaskServerError(http.StatusInternalServerError, err))
